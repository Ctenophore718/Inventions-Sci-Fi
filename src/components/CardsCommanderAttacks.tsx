...
import { generateTyrantSecondaryAttackStatsJSX } from '../utils/tyrantSecondaryAttack';
...
export const CardsCommanderAttacks: React.FC<CardsCommanderAttacksProps> = ({ sheet }) => {
  ...
  const blasters = sheet?.blasters || [];
  
  if (rifles.length === 0 && whips.length === 0 && sabres.length === 0 && flares.length === 0 && blasters.length === 0) {
    return null;
  }
  ...
  return (
    <>
      ...
      {/* Tyrant Blaster Secondary Attack Cards */}
      {sheet?.subclass === 'Tyrant' && Array.isArray(sheet?.blasters) && sheet.blasters.length > 0 && sheet.blasters.map((blaster, idx) => (
        <div key={blaster + idx} style={{
          width: '240px',
          height: '336px',
          background: '#fff',
          border: '5px solid #990000',
          ...
          <img 
            src={blaster === 'Blizzard Blast' ? '/Blizzard Blast.png' : blaster === 'Shock Gun' ? '/Shock Gun.png' : '/Blank Card.png'}
            alt={blaster}
            style={{
              position: 'absolute',
              top: 35,
          ...
          <div style={{
            ...
            <span style={{ color: '#990000', fontFamily: 'Arial, Helvetica, sans-serif', fontWeight: 'bold', fontSize: '1.1em', textAlign: 'left' }}>Secondary Attack</span>
            <span style={{ color: '#990000', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '0.875em', fontStyle: 'italic', marginRight: 22, whiteSpace: 'nowrap', maxWidth: 'calc(100% - 120px)', overflow: 'hidden', textOverflow: 'ellipsis', textAlign: 'right' }}>
              Cooldown <span style={{ fontWeight: 'bold', fontStyle: 'normal' }}>[{4 - ((sheet?.subclassProgressionDots as any)?.tyrantAttackCooldownDots?.filter(Boolean).length || 0)}]</span>
            </span>
          </div>
          <div style={{
            position: 'absolute',
            ...
            <div style={{
              ...
              {generateTyrantSecondaryAttackStatsJSX(
                (sheet?.subclassProgressionDots as any)?.tyrantAttackDamageDots,
                (sheet?.subclassProgressionDots as any)?.tyrantAttackCritDots,
                (sheet?.subclassProgressionDots as any)?.tyrantAttackCooldownDots,
                blaster
              )}
            </div>
          </div>
          <div style={{
            ...
            {blaster === 'Blizzard Blast' ? '"There\'s just something about a nasty winter storm that just makes you wanna take a nap, amirite?" --Viktor Houndsen, Canid Tyrant' 
            : blaster === 'Shock Gun' ? 'Like its namesake, the Shock Gun is designed to paralyze its targets and simply electrify them to a nice, crispy grave.' : 'Blaster effect.'}
          </div>
        </div>
      ))}
    </>
