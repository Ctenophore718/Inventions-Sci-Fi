...
const LevelUpBackground: React.FC<LevelUpBackgroundProps> = ({ 
  ...
  const safeCloneBackgroundDots = (): CharacterSheet["backgroundProgressionDots"] => {
    ...
    return {
      pollenCollectiveGreenThumbDots: sheet.backgroundProgressionDots.pollenCollectiveGreenThumbDots ? [...sheet.backgroundProgressionDots.pollenCollectiveGreenThumbDots] : [false]
    };
  };
  ...
  const persistBackgroundDots = (newDots: CharacterSheet["backgroundProgressionDots"], spDelta: number) => {
    if (onAutoSave) {
      ...
      if (setSpSpent) {
        setSpSpent((prev: number) => Math.max(0, prev + spDelta));
      }
    }
  };
        ...
        <div style={{ fontSize: '1em', color: '#000', marginBottom: '8px', fontFamily: 'Arial, Helvetica, sans-serif', marginTop: '20px' }}>
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', gap: '12px' }}>
            <span style={{ display: 'inline-block', maxWidth: 'calc(100% - 40px)' }}>
              <b><i style={{ color: '#6b7d1a' }}>Green Thumb.</i></b> Your training within the Pollen Collective has allowed you to become adept at planting seeds and maintaining any flora from gardens to entire forests. Gain an advantage on skills related to the growth and maintenance of the natural world.
            </span>
            <div style={{
              display: 'grid',
  ...
  return <div style={{ padding: '1rem', fontSize: '1em', color: '#000', fontFamily: 'Arial, Helvetica, sans-serif' }}>
